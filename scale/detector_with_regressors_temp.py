...
class DetectorWithRegressors(nn.Module):
    def __init__(
        ...
        self.bbox_metrics = BboxMetrics("argoverse")
        self.adaptive_crop = AdaptiveCrop("argoverse")
        ...
        if regressor_config is not None and "complexity" in regressor_config:
            ...
            self.complexity_regressor.load_state_dict(
                torch.load(regressor_config["complexity"]["checkpoint_file"])
            )
    ...
    def forward(self, data, get_metrics=False, im_shape=None):
        ...
        if get_metrics:
            ...
            class_info = self.bbox_metrics.get_class_info_metric(bbox_results[0])
            conf_info = self.bbox_metrics.get_confidence_metric(bbox_results[0])
            tb_lr_crop = self.adaptive_crop.get_crop_extents((im_shape[1], im_shape[0]), bbox_results[0])

            switch_temp = self.complexity_regressor(input_feat)
            # Converting into probs: [0.1,0.2,0.7]
    ...
    def detect(self, image, get_metrics=False):
        with torch.no_grad():
            data = self.preprocess_image(image)
            result = self.forward(data, get_metrics=get_metrics, im_shape=image.shape)
            return result
...
if __name__ == "__main__":
    ...
    proposal_vals = [50, 100]
    scale_vals = [(2000, 480), (2000, 360)]
