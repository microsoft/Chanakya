...
from chanakya.RL_gpu_aditya import MultiOutputMAB
            ...
            if type(fidx) is list:
                ...
                if len(fidx) == 2:
                    ...
                    if fidx[0] == "load_model":
                        ...
                        rl_model.plot("impt_plots",
                        prefixes = [ "{}_{}_prop".format(opts.rl_config,  opts.dataset), "{}_{}_scale".format(opts.rl_config,  opts.dataset), "{}_{}_model".format(opts.rl_config,  opts.dataset)], 
                        action_sets = [ 
                                [ str(x) for x in mab_configs[opts.rl_config]["actions"][0][1] ], 
                                [ str(x[-1]) for x in mab_configs[opts.rl_config]["actions"][1][1] ],
                                [ str(x) for x in mab_configs[opts.rl_config]["actions"][2][1] ]
                            ]
                        )
                        exit("Plotted")
                        print(1/0)
