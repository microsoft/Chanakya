...
def det_process(opts, frame_recv, det_res_send, w_img, h_img, config, client_state):
    ...
    try:
        ...
        # model.change_num_proposals(300)
        model.change_scale((2000, 600))
        ...
        while 1:
            ...
            t2 = perf_counter() 
            t_send_frame = t2 - t1
            result, predicted_scale = model.detect(img, predict_scale=True)
            model.change_scale((2000, predicted_scale))
            parsed_result = model.parse_result_for_sap(result)
            torch.cuda.synchronize()
            t3 = perf_counter()
            det_res_send.send([parsed_result, t_send_frame, t3, t3-t2, predicted_scale])
...
def main():
    ...
    if opts.dynamic_schedule and opts.dynamic_schedule_type != "mean":
        with open("/FatigueDataDrive/HAMS-Edge/sim/rtfs/{}_{}_adascale.pkl".format(opts.model_name, opts.dataset), "rb") as f:
    ...
    if opts.dynamic_schedule:
        if opts.dynamic_schedule_type == "mean":
            ...
            mean_rtf = mean_rtfs[600]*opts.fps*opts.perf_factor
            ...
            while fidx is not None:
                ...
                if det_res_recv.poll(wait_time): # wait
                    ...
                    if len(result) == 4:
                        ...
                        result, t_send_frame, t_start_res, t_det, predicted_scale = result
                    ...
                    if opts.dynamic_schedule:
                        if opts.dynamic_schedule_type == "mean":
                            ...
                            mean_rtf = mean_rtfs[(predicted_scale//10)*10]*opts.fps*opts.perf_factor
