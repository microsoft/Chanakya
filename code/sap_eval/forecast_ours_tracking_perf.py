...
def det_process(opts, frame_recv, det_res_send, w_img, h_img, config, client_state):
    ...
    try:
        ...
        tracker = Tracktor(
            ...
            "cuda:0",
            adaptive_crop=True,
            adaptive_crop_mode="discard"
        )
        tracker.change_scale((2000, 600))
        tracker_stride = 5
        ...
        tracker_scales_choices =  mab_configs[opts.rl_config]["actions"][2][1]
        tracker_stride_choices = mab_configs[opts.rl_config]["actions"][3][1]
        ...
        tracker.change_scale(tracker_scales_choices[init_action[2]])
        tracker_stride = tracker_stride_choices[init_action[3]]
        ...
        while 1:
            ...
            if k > 0 and k % rl_stride == 0:
                ...
                # print(k, fidx, prop_choices[actions[0]], det_scales_choices[actions[1]])
            else:
                if run_det % tracker_stride == 0:
                    result = model.detect(img) #, get_metrics=True)
                else:
                    try:
                        result = tracker.track(img, unparsed_result)
                    except:
                        result = copy.deepcopy(unparsed_result)

            unparsed_result = copy.deepcopy(result)            
            parsed_result = model.parse_result_for_sap(result)
            torch.cuda.synchronize()

            t3 = perf_counter()
            if k > 0 and k % rl_stride == 0:
                det_res_send.send([parsed_result, t_send_frame, t3, t3-t2, [1, 1] ])
            else:
