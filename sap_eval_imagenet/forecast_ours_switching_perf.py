...
def parse_args():
    ...
    parser.add_argument('--rl-model-folder', type=str, required=True)
    parser.add_argument('--rl-model-prefix', type=str, required=True)
...
def det_process(opts, frame_recv, det_res_send, w_img, h_img, config, client_state):
    ...
    try:
        ...
        models = {
            "faster_rcnn" : DetectorWithRegressors(
                "faster_rcnn",
                dataset,
                models_info[dataset]["faster_rcnn"]["config_file"],
                models_info[dataset]["faster_rcnn"]["checkpoint_file"],
                "cuda:0",
                models_info[dataset]["faster_rcnn"]["regressors"]
            ),
            "fcos" : DetectorWithRegressors(
                "fcos",
                dataset,
                models_info[dataset]["fcos"]["config_file"],
                models_info[dataset]["fcos"]["checkpoint_file"],
                "cuda:0",
                None
            ),
            "yolov3" : DetectorWithRegressors(
                "yolov3",
                dataset,
                models_info[dataset]["yolov3"]["config_file"],
                models_info[dataset]["yolov3"]["checkpoint_file"],
                "cuda:0",
                None
            )            
        }

        rl_config = mab_configs[opts.rl_config]
        state_size = rl_config["states"]["state_size"]
        prop_choices = rl_config["actions"][0][1]
        det_scales_choices = rl_config["actions"][1][1]
        switch_choices = mab_configs[opts.rl_config]["actions"][2][1]
        init_action = mab_configs[opts.rl_config]["rl"]["init_action"]
        rl_model = MultiOutputMAB(rl_config, device="cuda:0")
        rl_model.load(opts.rl_model_folder, opts.rl_model_prefix)

        for key in models.keys():
            models[key].change_num_proposals(prop_choices[init_action[0]])
            models[key].change_scale(det_scales_choices[init_action[1]])
        
        curr_model = switch_choices[init_action[2]]

        rl_stride = 20

        # warm up the GPU
        for key in models.keys():
            _ = models[key].detect(np.zeros((h_img, w_img, 3), np.uint8))
        ...
        while 1:
            ...
            if type(fidx) == tuple and fidx[0] == 'wait_for_ready':
                eval_client.change_im_shape(fidx[1])
            ...
            if k % rl_stride == 0:
                result, metrics = models["faster_rcnn"].detect(img, get_metrics=True, get_switch_metric=True, get_area_metric=True)
                actions = rl_model.act(metrics, test_phase=True)
                curr_model = switch_choices[actions[2]]
                for key in models.keys():
                    models[key].change_num_proposals(prop_choices[actions[0]])
                    models[key].change_scale(det_scales_choices[actions[1]])
                print(k, curr_model, prop_choices[actions[0]], det_scales_choices[actions[1]])
            else:
                result = models[curr_model].detect(img) #, get_metrics=True)
            
            parsed_result = models[curr_model].parse_result_for_sap(result)
            torch.cuda.synchronize()

            t3 = perf_counter()
            if k % rl_stride == 0:
                det_res_send.send([parsed_result, t_send_frame, t3, t3-t2, actions ])
            else:
                det_res_send.send([parsed_result, t_send_frame, t3, t3-t2])
            k += 1
        try:
            rl_model.features_importance(opts.rl_model_folder)  ##Please check!!
        except:
            print("Error while calculating feature importance")
...
def main():
    ...
    mean_rtfs = {}
    for model_name in switch_choices:
        with open("/FatigueDataDrive/HAMS-Edge/sim/rtfs/{}_{}.pkl".format(model_name, opts.dataset), "rb") as f:
            mean_rtf = pickle.load(f)
        for x in mean_rtf.keys():
            mean_rtf[x] = np.mean(mean_rtf[x])
        mean_rtfs[model_name] = copy.deepcopy(mean_rtf)
    # dynamic scheduling
    if opts.dynamic_schedule:
        # mean_rtf = 0.0
        mean_rtf = mean_rtfs[switch_choices[init_action[2]]][ prop_choices[init_action[0]], det_scales_choices[init_action[1]][1] ]*opts.fps*opts.perf_factor
    ...
    with torch.no_grad():
        ...
        for sid, seq in tqdm(seqs):
            ...
            eval_client.change_im_shape(im_shape)
            ...
            frame_send.send( ('wait_for_ready', im_shape) )            
            while 1:
            ...
            while fidx is not None:
                ...
                if det_res_recv.poll(wait_time): # wait
                    ...
                    if len(result) == 4:
                        ...
                        result, t_send_frame, t_start_res, t_det, actions = result
                        new_action = True
                    if opts.dynamic_schedule and new_action:
                        mean_rtf = mean_rtfs[switch_choices[actions[2]]][ prop_choices[actions[0]], det_scales_choices[actions[1]][1] ]*opts.fps*opts.perf_factor
                        new_action = False
